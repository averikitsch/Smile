...
% The update plot function, which should be changed to show what needs to be shown        
        function UpdatePlot(self)
            ...
            subplot(1,2,1); plot(self.data(:,self.targetChannel));%surf(self.data);
            %axis([0 140 4200 4700]);
            sigFilter=fft(self.data(:,self.targetChannel));
            magF=abs(sigFilter);
            angF=unwrap(angle(sigFilter));
            tt=linspace(-self.sampFreq/2,self.sampFreq/2,length(self.data));
            %set(0,'CurrentFigure',self.fig_h2)
            subplot(1,2,2); semilogy(tt,fftshift(magF));
            %axis([0 40 0 100]);
            xlabel('Frequency (Hz)','fontsize',12);
            ylabel('Magnitude','fontsize',12);
%             if isempty(self.fig_h2)
%                 self.fig_h2 = figure;
%             end
%             [vNew,aNew,emotion]=emotionType(self);
%             set(0,'CurrentFigure',self.fig_h2)
%             img = imread('emotion.png');
%             imagesc([0 10],[0 10], flipdim(img,1));
%             hold on;
%             plot(self.arousal,self.valence,'ko',aNew,vNew,'bo','MarkerSize',12);
%             set(gca,'ydir','normal');
%             axis([0 10 0 10]);
%             hold off;
...
%             end
    ...
    %% Band Power
    function ratio = powerRatio(self, channel)
        channelIndex = ismember(fieldnames(self.DataChannels),channel);
        sigFilter=fft(self.data(:,channelIndex(:))); %can use find() instead
        magF=abs(sigFilter);
        beta = sum(magF(77:95));
        alpha = sum(magF(73:77));
        %beta = bandpower(self.data(:,channelInd),self.sampFreq,[12 30]);
        %alpha = bandpower(self.data(:,channelInd),self.sampFreq,[8 12]);
        ratio = beta/alpha;
    end
    function [valence,arousal,emotion] = emotionType(self)
        valence = powerRatio(self,'ED_AF4');
        arousal = powerRatio(self,'ED_F3');
        if (valence >= 5)
            if (arousal >= 5)
